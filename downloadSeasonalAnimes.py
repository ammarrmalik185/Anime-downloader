...
from Cores.Core_AnimeKisa import *
...
options = Options(
    ...
    download_path="D:\\Animes\\Seasonal Animes",
    # your download path
    pages_to_scan=2,
    # how many pages should the program scan for animes
    quality={"type": "force_res", "value": "source"},
...
def downloaded_anime_check():
    if os.path.isfile(r"cache\recently downloaded seasonal animes.txt"):
        file = open(r"cache\recently downloaded seasonal animes.txt", "r", encoding='utf-8')
        ...
        if not os.path.exists(r"cache"):
            os.makedirs(r"cache")
        open(r"cache\recently downloaded seasonal animes.txt", "w", encoding='utf-8')
        run_1[0] = True


def valid_anime_check():
    if os.path.isfile(r"cache\seasonal animes.txt"):
        file = open(r"cache\seasonal animes.txt", "r", encoding='utf-8')
        for line in file.readlines():
            if "\n" in line:
                line = line.replace("\n", "")
            if line != "" and line != " ":
        ...
        if not os.path.exists(r"cache"):
            os.makedirs(r"cache")
        open(r"cache\seasonal animes.txt", "w", encoding='utf-8')
        run_1[0] = True
...
def save(downloaded_anime_name, ep):
    ...
    while True:
        try:
            ...
            file = open(r"cache\recently downloaded seasonal animes.txt", "w", encoding='utf-8')
            file.write(text)
            file.close()
            break
...
def core():
    ...
    potentially_new_animes = []
    for i in info[2]:
        single_anime_string = ""
        if i["ep_no"] == "1" or i["ep_no"] == "0":
            single_anime_string += ("Name       : " + i["name"]) + "\n"
            single_anime_string += ("Episode no : " + i["ep_no"]) + "\n"
            potentially_new_animes.append(single_anime_string)

    if len(potentially_new_animes) != 0:
        file = open(r"cache\potential new animes seasonal.txt", "w", encoding='utf-8')
        print("-------------------------Potential New Animes--------------------------\n")
        for i in potentially_new_animes:
            file.write(i + "\n")
            print(i)
        file.close()
