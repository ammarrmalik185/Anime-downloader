...
class Crawler:
    ...
    def initialize_driver(self):
        ...
        self.driver_options = EdgeOptions()
        self.driver_options.use_chromium = True
        ...
        self.driver_options.add_argument(
            'user-data-dir=C:\\Users\\Ammar Malik\\AppData\\Local\\Microsoft\\Edge\\User Data')
    ...
    def get_initial_data_from_url(self, url):
        ...
        for exception_count in range(self.options.max_refresh_page):
            try:
                ...
                # log - exception
                exception_statement = "Unable to load the main page, error: " + ex.msg + ", trying again attempt no :" \
        ...
        if not success[0]:
            # log - exception
            exception_statement = "Failed to load the main page after max refreshes"
            self.errors.append(exception_statement)
            self.logs.append("[" + str(datetime.now().time()) + "]:" + exception_statement)
            if self.options.logs:
                print(exception_statement)
    ...
    def filter_anime_data(self, anime_data):
        ...
        for anime in anime_data:
            ...
            if (anime["name"].lower() in self.already_downloaded_animes) and (anime["ep_no"] ==
                                                                              self.already_downloaded_eps[
                                                                                  self.already_downloaded_animes.index(
                                                                                      anime["name"].lower())]):
                downloaded = True
    ...
    def get_download_link_per_anime(self, link):
        ...
        # self.driver.execute_script("downloadEpisode()")
        handles = self.driver.window_handles
        self.driver.close()
        self.driver.switch_to.window(handles[1])

        # print(self.driver.find_element_by_tag_name("title"))
        print(self.driver.title)
        if self.driver.title == "Attention Required! | Cloudflare":
            input("solve captcha and press enter")

        try:
            download_link = self.driver.current_url
            if download_link.startswith("https://gogo-play.net/streaming.php?"):
                download_link = download_link.replace("streaming.php?", "download?")
    ...
    def choose_from_quality(self, qualities):
        ...
        if self.options.quality["type"] == "force_res" and self.options.alternate_quality:
            if qualities.get(qualities.keys()[0]) is not None:
                return qualities[qualities.keys()[0]], "200"
            else:
                return "none", "404"
            # if 'DOWNLOAD (HDP - MP4)' in qualities:
            ...
            for exception_count in range(self.options.max_refresh_page):
                try:
                    ...
                    exception_statement = "Error occurred loading page of anime: " + anime_data["name"] + ", url: " + \
                                          anime_data["link"] + ", error: " + str(
                        ex.args[0]) + ", trying again attempt no :" + \
                                          str((exception_count + 1)) + " out of " + str(self.options.max_refresh_page)
                    self.errors.append(exception_statement)
                    self.logs.append("[" + str(datetime.now().time()) + "]:" + exception_statement)
